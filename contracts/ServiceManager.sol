...
contract ServiceManager {
    ...
    modifier validProvidersOnly(address _provider) {
        ...
        require(
            serviceProviderIndexes[serviceProviders[_provider].index] ==
                _provider,
    ...
    function getServiceProviders()
        ...
        returns (ServiceProvider[] memory)
    {
        ServiceProvider[] memory validServiceProviders = new ServiceProvider[](
            serviceProviderIndexes.length
        );

        for (uint256 i = 0; i < serviceProviderIndexes.length; i++) {
    ...
    function createServiceAgreement(
        address _provider
    ) external validProvidersOnly(_provider) {
        require(
